...
describe('Room Members API', () => {
    ...
    describe('Endpoints', () => {
        ...
        test('Should delete user from room', async () => {
            const roomId = 'Cuz3gcHNjdEqvXbqv';
            const userId = 'S3RsFtPzJaRfaT8mcDYy';
            const { deleted } = await api.deleteUser(roomId, userId);
    ...
    describe('Authorization', () => {
        test('Shoudl block user from inserting user in a non-owner room', async() => {
        ...
        test('Should block user from removing user from non-owned room', async() => {
           ...
           await expect(api.deleteUser(roomId, userId)).rejects.toThrow(/401/);
        });
    });
